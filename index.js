...
app.get("/", (req, res) => {
  try {
    ...
    // Verificar si hay parámetros inmediatos después de la raíz:
    if (nombre || precio) {
  ...
  res.sendFile(__dirname + "/index.html"); //muestra el index
});
...
app.get("/agregar", (req, res) => {
  ...
  if (!nombre || !precio) {
    console.log(
      "Calma, calma... Faltan parámetros, se requiere nombre y precio"
    );
    return res.send(
      "Calma, calma... Faltan parámetros, se requiere nombre y precio"
    ); //los res.send aparecen como alerts, de lo contrario sólo aparece mensaje en terminal/consola
  }
  ...
  // Verificar si el nombre del deporte ya existe en la lista de deportes
...
//-------------------------------------------------------------------------------------
// Ruta genérica, control de ruta no existentes
app.get("*", (req, res) => {
